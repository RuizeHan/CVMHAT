...
segment_num = size(tv_midLevelTracklets,2) + 1;
random_itrack_prec = 0.1;
random_itrack = segment_num * random_itrack_prec;
...
for iTrack = 1:length(tv_midLevelTracklets)-1
    ...
    elseif iTrack == 1 && random_noisy == 1
        random_prec = 1;
        rand_hv_tracklet_num = floor(random_prec * hv_numOldTracklets);
        mha_hv_tracklet_num = hv_numOldTracklets - rand_hv_tracklet_num;
        cv_midLevelTracklets{1}(1,iTrack).IDS = 1:tv_numOldTracklets;
        hv_IDS_idx = 1:hv_numOldTracklets;
        hv_IDS = 1:hv_numOldTracklets;
        hv_IDS(hv_flag_num) = tv_flag_num;
        ...
        other_ID = setdiff(hv_IDS_idx,hv_flag_num);
        hv_IDS(other_idx) = other_ID;   
        mha_hv_tracklet_id = hv_IDS(1:mha_hv_tracklet_num);
        rand_id = setdiff(cv_midLevelTracklets{1}(1,iTrack).IDS, mha_hv_tracklet_id);
        rand_id= rand_id(randperm(length(rand_id)));
        rand_hv_tracklet_id = rand_id(1:rand_hv_tracklet_num);
        hv_IDS = [mha_hv_tracklet_id, rand_hv_tracklet_id];      
        cv_midLevelTracklets{2}(1,iTrack).IDS = hv_IDS; 
        maxID = max(tv_numOldTracklets,hv_numOldTracklets);   
    end
    ...
    % reinitialize the hv_IDS by tv_IDS every 10 segments 
    if  mod(iTrack,re_initial) == 0 && iTrack > 1 
     ...
     if mod(iTrack,5) == 0 && random_noisy == 1
        %%% random prec
        random_prec = 1;
        rand_hv_tracklet_num = floor(random_prec * hv_numOldTracklets);
        mha_hv_tracklet_num = hv_numOldTracklets - rand_hv_tracklet_num;
        ...
        rand_id = setdiff(cv_midLevelTracklets{1}(1,iTrack).IDS, mha_hv_tracklet_id);
        rand_id= rand_id(randperm(length(rand_id)));
        if length(rand_id) >= rand_hv_tracklet_num
            rand_hv_tracklet_id = rand_id(1:rand_hv_tracklet_num);
        else
            randset_all = 1:15;
            randset_expect_mha = setdiff(randset_all,mha_hv_tracklet_id);
            randset_expect_mha= randset_expect_mha(randperm(length(randset_expect_mha)));
            rand_hv_tracklet_id = randset_expect_mha(1:rand_hv_tracklet_num);
        end
%         rand_hv_tracklet_id = rand_id(1:rand_hv_tracklet_num);
        hv_IDS = [mha_hv_tracklet_id, rand_hv_tracklet_id];      
        cv_midLevelTracklets{2}(1,iTrack).IDS = hv_IDS; 
    ...
    matchedIDS = [];
        ...
        % Assign the ID of the unmatched tracklets by the other view or a new ID
...
%         Assign the ID of all the hor-view tracklets by the top view for the $ iTrack+1 $

%         if mod(iTrack,re_initial) == 0 && view_i == 2           
%            IDSNew = ones(1,numNewTracklets)*-1;
...
%             try
            if currID < 0 || currID > length(tracks) 
                continue;
            end

...
mha_data.size_img_top = size(mha_data.img_top_1);
    ...
    for frm_i = 1 : length(tv_detection)
        ...
        % current_angle = last_angle - pi/(params.search_num/2) * (params.search_num) + pi/(params.search_num/2) * (mha_data.max_idx);
        current_angle =  mod(data.last_angle - deg2rad(params.search_num/2) + deg2rad(mha_data.max_idx),2*pi);
        
        [mha_data.vec_ego,mha_data.index_ego] = GoPro2vec(mha_data,params);

        mha_data = UAV2vec_one(mha_data.img_top,mha_data.objs_top,mha_data.cam_top,params,mha_data,current_angle); 

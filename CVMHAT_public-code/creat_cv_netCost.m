...
%---------------------------new app cost-----------------------
if use_app ==1
    app_path = [param_tracklet.data_directory, 'AppCost_New_v1/', param_tracklet.sceneName];
%     tracklets_dis_App = struct2cell(load(fullfile(app_path,['app_net_cost_',param_tracklet.sceneName,'.mat'])));
%     tracklets_dis_App = tracklets_dis_App{1};
else
    param_cv_netCost.Sweight = 1.0;
...
if save_tracklet == 1
    ...
    path = ['./Data/Tracklets_img/', param_tracklet.sceneName, '/'];
...
%             imshow(det);
            imwrite(det,strcat(savepath,'/',views{view_i},'_tra_',num2str(tra_i),'_fri_',num2str(frm_i),'.jpg'));
...
%-----------------new app cost---------
if use_app ==1
    tracklets_dis_App = struct2cell(load(fullfile(app_path,['seg_', int2str(seg_i),'.mat'])));
    % tracklets_dis_A = normalizing(tracklets_dis_App{1},0,1);
    tracklets_dis_A = 0.5 * tracklets_dis_App{1};
end
